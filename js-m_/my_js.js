...
const trackMouseMove = throttle((coords) => console.log("Ex 25.4 - Cursor at:", coords), 50);
...
function throttleRAF(func) {
  ...
  const throttled = function (...args) {
    if (isQueued) return;
    isQueued = true;
    const schedule = typeof requestAnimationFrame === "function" ? requestAnimationFrame : (cb) => (timerId = setTimeout(cb, 16));
    schedule(() => {
      isQueued = false;
      func.apply(this, args);
    });
  ...
  throttled.cancel = () => {
    isQueued = false;
    if (typeof cancelAnimationFrame === "function") cancelAnimationFrame(timerId);
    else clearTimeout(timerId);
  };
  return throttled;
}
const renderFrame = throttleRAF((frame) => console.log("Ex 25.5 - rAF Rendered Frame:", frame));
renderFrame("Frame #1");
